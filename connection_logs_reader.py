...
def print_connection(c, indent, timestamp):
	indentStr = ' ' # All the edits are by the intern:Abubaker
	timestamp_pattern = '%Y-%m-%d %H:%M:%S'	
	connection_info = str(timestamp)
	
	# if the timestamp is a string, it will be changed to epoch 
	if isinstance(timestamp, str):
		 connection_info = str( time.mktime(time.strptime(timestamp, timestamp_pattern)) )
	
	# Adding the connection info to the string
	if c['connection_type'] in ['accept', 'reject', 'pending']:
		connection_info += indentStr + 'connection {:d} {:s} {:s} {:s} {:s}:{:d} <- {:s}:{:d}'.format(
		...
		connnection_info += indentStr + 'connection {:d} {:s} {:s} {:s} {:s}:{:d} -> {:s}/{:s}:{:d}'.format(
		...
		connection_info += indentStr + 'connection {:d} {:s} {:s} {:s} {:s}:{:d}'.format(
			c['connection'], c['connection_protocol'],
			c['connection_transport'], c['connection_type'], c['local_host'],
			c['local_port'])
	...
	connection_info += ' ({} {})'.format(c['connection_root'], c['connection_parent']) + '\n'
	
	# if the connection was already there don't add it to the connections log file, else just append it to the connections log file
	if os.path.isfile("/opt/dionaea/var/log/connections_log.txt"):
		connectionLogs_file = open("/opt/dionaea/var/log/connections_log.txt", "r")
		if connection_info in connectionLogs_file.read():
			connectionLogs_file.close()
			return
	
	connectionLogs_file = open("/opt/dionaea/var/log/connections_log.txt", "a")
	send_to_elasticsearch.send_to_es(connection_info) # sending to elasticsearch
	connectionLogs_file.write(connection_info)
	connectionLogs_file.close()
...
def recursive_print(cursor, connection, indent):
	...
	for c in connections:
		...
		print_connection(c, indent+1, c['connection_timestamp'])
...
#		print(connections)
		for c in connections:
			...
			#print(c['connection_timestamp'])  # Connection Time Stamp
			connection_timestamp = "{:s}".format(c['connection_timestamp'])
			print_connection(c, 1, connection_timestamp)
...
# Keep updating the connection logs text file every 1 second
while(1):
	parser = OptionParser()
